...
•	Trạng thái khởi đầu
o	Với AND-OR Search: Xuất phát từ một bảng cụ thể 3x3 gồm các số từ 1 đến 8 và một ô trống (0), ví dụ: [[1, 2, 3], [4, 5, 6], [7, 0, 8]].
o	Với Belief State Search: Khởi tạo bằng một tập hợp belief states gồm trạng thái gốc và hai trạng thái kề cạnh, phản ánh các khả năng chuyển đổi trong môi trường không quan sát được.
o	Với Partial Observable Search (POS): Bắt đầu từ tập các belief states có điểm chung là số 1 nằm tại vị trí (0,0) – được hình thành nhờ khả năng quan sát một phần trạng thái ban đầu và trạng thái lân cận.
•	Trạng thái mục tiêu
...
o	Ô tại vị trí (0,0) bắt buộc phải là số 1.
o	Mỗi số từ 0 đến 8 chỉ xuất hiện đúng một lần trên toàn bảng.
o	Ràng buộc theo hàng: ô bên phải phải có giá trị lớn hơn ô bên trái đúng 1 đơn vị (trừ ô trống).
o	Ràng buộc theo cột: ô phía dưới phải lớn hơn ô phía trên đúng 3 đơn vị (trừ ô trống).
o	Cấu hình cuối cùng phải là trạng thái có thể giải được (tức số nghịch đảo là chẵn).
•	Hành động thực hiện
Hệ thống gán từng giá trị vào ô, sử dụng:
o	Backtracking Search: duyệt tuần tự, quay lui nếu gặp xung đột.
o	Forward Checking: mỗi lần gán sẽ đồng thời loại bỏ các giá trị không hợp lệ khỏi domain của các ô khác, giúp tăng hiệu quả và tránh xung đột sớm.
•	Chi phí
...
o	MRV (Minimum Remaining Values): chọn ô có ít giá trị hợp lệ nhất để gán trước.
o	LCV (Least Constraining Value): ưu tiên gán giá trị ít ảnh hưởng đến ô khác nhất.
